...
bookRoute.get("/book/:id",async(req,res)=>{
  ...
  try {
    ...
    console.log(error)
    res.status(StatusCodes.BAD_REQUEST).json({msg:"Internal server failure"})
  }
});
bookRoute.post("/book",async(req,res)=>{
  try {
    ...
    if(!title||!author||!year){
      return res.status(StatusCodes.ACCEPTED).json({msg:"Please provide title author year"});
    }
    await Book.create(req.body);
    res.status(StatusCodes.OK).json({msg:"Book Added SuccessFully"});
  } catch (error) {
    console.log(error)
    res.status(StatusCodes.BAD_GATEWAY).json({msg:"Internal Server Failure"});
  }
});
...
bookRoute.put("/book/:id", async(req,res)=>{
  ...
  try {
    ...
    if(!author||!title||!year){
      return res.status(StatusCodes.ACCEPTED).json({msg:"please provide all details"})
    }
    const result = await Book.findByIdAndUpdate(id,req.body);
    if(!result){
      return res.status(StatusCodes.NOT_FOUND).json({msg: `Book not found with id-${id}`});
