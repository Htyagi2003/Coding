...
int main(){
    cout<<"hello world\n";
    array<int,6>arr={0};// when we have to declare array with all zeroes in it.
    arr.fill(5);// to fill the element with single value at every index.
    // cout<<arr.at(3); // for accesing ith index element
    ...
    // begin-> starting point of array or any container.
    //rbegin->reverse begin->last element of any container.
    //end-> right after the finishing element or empty adress after the last element.
    //rend-> right before the start element. or empty adress before the first element.
    array<int,5>arrr={1,2,3,4,5};
   ...
   cout<<endl;
    for(auto it=arrr.rbegin(); it!=arrr.rend();it++){
        cout<<*it<<" ";// for reverse printing the array.rbegin and r end are reverse iterators.
    }
   cout<<endl;
   for(auto it=arrr.end()-1; it>=arrr.begin();it--){
        cout<<*it<<" ";// for reverse printing the array this method also give the same.
    }
   cout<<endl;
   for (auto it:arrr)
   {
    cout<<it<<" ";// this is for each loop;
   }
   cout<<endl;
   string s="harsh tyagi";
   for(auto c:s){
    cout<<c;// this is the use of for each loop.
   }
   cout<<endl;
    cout<<arrr.size()<<endl;//to get size 
    cout<<arrr.front()<<endl;// to get first element arr.at(0)
    cout<<arrr.back()<<endl;// to get last element arr.at(arr.size()-1)
    // VECTOR 
    ...
    cout<<endl;
    huu.clear();// erase all the element
    huu.push_back(7);
    for(auto i:huu){
        cout<<i<<" ";
    }
    ...
    vector<int>vec2(vec.begin(),vec.end());// 1st method to copy.
    for(auto i:vec2){
        cout<<i<<" ";
    }

    cout<<endl;
    vector<int>vec3(vec2);// copy vec2 into vec3
    for(auto i:vec3){
        cout<<i<<" ";
    }
